...
class ServerHttp {
  ...
  chatwootCtrl = async ( req, res ) => {
    ...
    try {
       ...
       */
      const checkIfMessage = body?.private == false && body?.event == "message_created" && ( body?.message_type === "outgoing" || body?.message_type === "template" ) && body?.conversation?.channel.includes( "Channel::Api" );
      if ( checkIfMessage ) {
